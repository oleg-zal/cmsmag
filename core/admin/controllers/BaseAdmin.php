...
abstract class BaseAdmin extends BaseController
{
    ...
    protected function emptyFields($str, $answer, $arr=[]) {
        if (empty($str)) {
            $_SESSION['res']['answer'] = '<div class="error">' . $this->messages['empty'] .
                ' '.
                $answer . '</div>';
            $this->addSessionData($arr);
        }
    ...
    protected function editData($returnId=false) {
        ...
        $idRow = $_POST[$this->columns['id_row']];
        if ($idRow) {
            $id = is_numeric($idRow) ? $this->clearNum($idRow) : $this->clearStr($idRow);
            if($id) {
    ...
    protected function createAlias($id=false) {
        if ($this->columns['alias']) {
            ...
            $testModify = new TextModify();
            $alias = $testModify->translit($alias_str);
            $where['alias'] = $alias;
            $operand[] = '=';
            if ($id) {
                $where[$this->columns['id_row']] = $id;
            ...
            foreach ($manyToMany as $mTable => $tables) {
                ...
                if ($targetKey !== false) {
                    ...
                    if ($this->data) {
                        ...
                        if ($res) {
                            foreach ($res as $item) {
                                $foreigh[] = $item[$tables[$otherKey] . '_' . $orderData['columns']['id_row']];
                            }
                        }
                    }
                                ...
                                while (($key = key($data)) !== null) {
                                    if (!$data[$key]['parent_id']) {
                                        ...
                                        if ($this->foreignData[$tables[$otherKey]][$data[$key][$orderData['parent_id']]]) {
                                            ...
                                            if (in_array($data[$key]['id'], $foreigh)) {
                                            ...
                                            foreach ($this->foreignData[$tables[$otherKey]] as $id => $item) {
                                                ...
                                                if ( !empty($item['sub']) && isset($item['sub'][$parent_id]) ) {
    ...
    protected function createMenuPosition($settings = false) {
        if ($this->columns['menu_position']) {
            ...
            $iteration = (int) !$this->data;
            $menu_pos = $res[0]['count'] + $iteration;
